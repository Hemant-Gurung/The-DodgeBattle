...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (!m_pVertexBuffer || m_Sprites.size() > m_BufferSize)
	{
		...
		bd.ByteWidth = sizeof(VertexSprite) * (UINT32)m_Sprites.size();
	...
	if (m_pVertexBuffer)
	{
		...
		HRESULT hResultR =sceneContext.d3dContext.pDeviceContext->Map(m_pVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &resources);

		if (hResultR != S_OK)
			return;
		// use memcpy to copy all our sprite vertices (m_Sprites) to the mapped resource (D3D11_MAPPED_SUBRESOURCE::pData)
		std::memcpy(resources.pData, m_Sprites.data(),sizeof(VertexSprite));
