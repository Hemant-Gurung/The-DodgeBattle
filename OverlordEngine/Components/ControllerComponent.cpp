...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		TODO_W5(L"Complete the ControllerComponent Intialization")
			//PhysxProxy::G
			auto position = m_pGameObject->GetComponent<TransformComponent>()->GetPosition();
		
		    m_ControllerDesc.position = PxExtendedVec3(position.x,position.y,position.z);
			m_ControllerDesc.radius = 5;
			m_ControllerDesc.height = 20;
		
			//RETRIEVE PHYSICS PROXY SCENE
		m_pController = m_pScene->GetPhysxProxy()->GetControllerManager()->createController(m_ControllerDesc);
		ASSERT_NULL(m_pController,nullptr);
		m_pController->setUserData(m_ControllerDesc.userData);
		auto m = m_CollisionGroups.word0;
		
		//CAST TO COLLISION GROUP
		SetCollisionGroup(static_cast<CollisionGroup>(m));
