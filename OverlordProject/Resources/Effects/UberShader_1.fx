...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specColor = gTextureSpecularIntensity.Sample(gTextureSampler,texCoord)*specularStrength;
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float3 specColor = gTextureSpecularIntensity.Sample(gTextureSampler,texCoord)*specularStrength;
	phong = specColor;
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	{
		sampledNormal = gTextureNormal.Sample(gTextureSampler,texCoord);
	}
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	float3 diffuseColor = gColorDiffuse.rgb * diffuseStrength; 
